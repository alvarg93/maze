...
int direction = 0;
int current_elevator_i;
int current_elevator_j;

glm::vec3 elevator_light_pos_down;
glm::vec3 elevator_light_pos_up;
...
void drawObject() {
	...
	glUniform4f(shaderProgram->getUniformLocation("pos"), m_eye.x, m_eye.y, m_eye.z, 1);// m_eye.x, m_eye.y, m_eye.z, 1);
	...
	glUniform4f(shaderProgram->getUniformLocation("elevator_pos_up"), elevator_light_pos_up.x, elevator_light_pos_up.y+4.0f, elevator_light_pos_up.z, 1);

	glUniform4f(shaderProgram->getUniformLocation("elevator_pos_down"), elevator_light_pos_down.x, elevator_light_pos_down.y, elevator_light_pos_down.z, 1);
...
{
	direction = 1;
	current_elevator_i = p_i;
	current_elevator_j = p_j;
};
...
void DrawFloor(int layer){
	...
	for (int j = 0; j < maze_size; j++){
		...
		//if (M[i][j] == 'd') current_elevator_i = i, current_elevator_j = j;
		if (M[i][j] != 'd' || direction==0)
		for (int kx = 0; kx < 2; kx++)
		for (int kz = 0; kz < 2; kz++)
		{
			if (M[i][j] == 'u' || M[i][j] == 'd'){
				...
				if(M[i][j] == 'u') elevator_light_pos_down = glm::vec3(4.0f*i + 1.0f, layer*4.0f, 4.0f*j + 1.0f);
				else elevator_light_pos_up = glm::vec3(4.0f*i + 1.0f, layer*4.0f, 4.0f*j + 1.0f);
			}
			matM = glm::translate(glm::mat4(1.0f), glm::vec3(4.0f*i + kx*2.0f, layer*4.0f - 2.0f, 4.0f*j + kz*2.0f));
			drawObject();
			if (M[i][j] == 'u' || M[i][j] == 'd'){
...
void DrawElevator(double step, double step_limit)
{

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, tex3);
	int i = current_elevator_i;
	int j = current_elevator_j;
	double layer = cur_layer;

	for (int kx = 0; kx < 2; kx++)
	for (int kz = 0; kz < 2; kz++)
	{
		matM = glm::translate(glm::mat4(1.0f), glm::vec3(4.0f*i + kx*2.0f, layer*4.0f - 2.0f + (direction*step/step_limit)*4.0f, 4.0f*j + kz*2.0f));
		drawObject();
	}
}
...
void DrawFlashlight() {
	...
	matM = glm::translate(matM, glm::vec3(0.10f, -0.10f, 1.55f));


	matM = glm::translate(matM, m_eye);

	matM = glm::scale(matM, glm::vec3(0.1f, 0.1f, 1.0f));
